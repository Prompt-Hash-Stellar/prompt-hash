...
use super::types::{
    ...
    PromptHashTrait, PromptPage, PurchaseDispute, Split, Escrow, EscrowState,
};
...
/// Delay between proposing a fee/referral change and it becoming activatable
/// (#82). Reuses the same 3-day duration already established elsewhere in
/// this file for the dispute appeal window.
const FEE_POLICY_TIMELOCK_SECS: u64 = 3 * 24 * 60 * 60;
...
impl PromptHashTrait for PromptHashContract {
    ...
    fn upgrade(env: Env, new_wasm_hash: BytesN<32>) -> Result<(), Error> {
        ...
        // Does NOT walk the whole market anymore (#83) — call
        // `extend_ttl_page` in bounded batches as a post-upgrade admin step
        // instead, so upgrades stay possible however large the market grows.
        Ok(())
    }
